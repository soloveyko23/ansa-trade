(() => {
    ...
    const detectSystemTheme = () => {
        if (window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches) setTheme(true); else setTheme(false);
    };
    ...
    const initTheme = () => {
        ...
        if (savedTheme !== null) setTheme(savedTheme === "true"); else detectSystemTheme();
        watchSystemTheme();
    };
    initTheme();
    ...
    const setupTariffsCheck = () => {
        const tariffsCheckBlocks = document.querySelectorAll(".tariffs-check");
        tariffsCheckBlocks.forEach((block => {
            const inputField = block.querySelector('input[name="form[]"]');
            const connectButton = block.querySelector(".button-green");
            if (inputField && connectButton) {
                ...
                const checkInput = () => {
                    ...
                    connectButton.disabled = isNaN(inputValue) || inputValue < .001;
    ...
    document.addEventListener("DOMContentLoaded", setupTariffsCheck);
    const script_select = document.querySelector('.select[data-id="1"]');
    const countStaking = document.getElementById("countStaking");
    const result = document.getElementById("result").querySelector("span");
    const MAX_VALUE = 21551e3;
    const calculateResult = () => {
        let count = parseFloat(countStaking.value.replace(/[^0-9.]/g, "") || 0);
        count = Math.min(count, MAX_VALUE);
        const selectedOption = script_select.querySelector(".select__option.selected") || script_select.querySelector(".select__option");
        const deadline = parseFloat(selectedOption.getAttribute("data-value"));
        const percentages = {
            30: .24,
            90: .98,
            180: 2.21,
        ...
        const percentage = percentages[deadline] || 0;
        const resultValue = Math.floor(count * percentage);
        result.textContent = resultValue.toLocaleString();
    };
    const updateResult = () => {
        calculateResult();
    };
    window.addEventListener("DOMContentLoaded", updateResult);
    countStaking.addEventListener("input", updateResult);
    script_select.addEventListener("click", (event => {
        const option = event.target.closest(".select__option");
        if (option || event.target.classList.contains("select__option")) {
            const selectedOption = script_select.querySelector(".select__option.selected");
